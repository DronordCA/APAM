  ...
  try {
    ...
    if (memberIdInput) {
      ...
      if (reserved && !normalizeEmail(reserved.values[USER_COL.EMAIL])) {
        ...
        const signupStatus = String(values[USER_COL.ACTIVE] || '').toUpperCase() === 'OUI' ? 'READY_TO_LOGIN' : 'PENDING_REVIEW';
        return jsonResponse_({ ok: true, message: 'signup_recorded', signupStatus: signupStatus, memberId: memberId });
      }
    ...
    return jsonResponse_({ ok: true, message: 'signup_recorded', signupStatus: 'PENDING_REVIEW', memberId: memberId });
  } finally {
    lock.releaseLock();
  }
}
