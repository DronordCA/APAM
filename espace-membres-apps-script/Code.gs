...
function doGet(e) {
  try {
    ...
    if (action === 'me') return handleMe_({ token: params.token || params.sessionToken || '' });
...
function handleMe_(payload) {
  ...
  const permissions = buildPermissions_(auth.user.values);
  ...
  return jsonResponse_({
    ...
    numero_membre: profile.MemberID,
    role: profile.Role,
    status: profile.Status,
    access_enabled: profile.AccessEnabled,
...
function buildPermissions_(values) {
  const permissions = [];
  if (String(values[USER_COL.COMPTE_PERSONNEL] || '').toUpperCase() === 'OUI') permissions.push('compte_personnel');
  if (String(values[USER_COL.DOC_CLUB] || '').toUpperCase() === 'OUI') permissions.push('docs_club');
  if (String(values[USER_COL.DOC_ELEVE] || '').toUpperCase() === 'OUI') permissions.push('docs_eleves');
